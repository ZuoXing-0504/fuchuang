...
def _normalize_probability(series: pd.Series) -> pd.Series:
    ...
    if float(max_value) == float(min_value):
        return pd.Series(np.where(numeric.notna(), 0.5, np.nan), index=series.index, dtype=float)
...
def _prepare_feature_table(feature_path: str | Path | None = None) -> pd.DataFrame:
    ...
    if path is None:
        raise FileNotFoundError("未找到特征表，请提供 feature_table.csv 或 train_features_final.csv。")
    ...
    feature_view = pd.DataFrame(
        {
            ...
            "college": _series_from_candidates(frame, ["college", "学院", "院系", "XSM", "DEPARTMENT_NAME", "YX", "DEPARTMENT"]),
            "major": _series_from_candidates(frame, ["major", "专业", "ZYM", "MAJOR_NAME", "ZY", "MAJOR"]),
            "study_time": study_time,
            "library_count": _series_from_candidates(frame, ["library_count", "图书馆次数", "图书馆访问次数", "进馆次数", "library_visit_count"]),
            "consume_avg": _series_from_candidates(frame, ["consume_avg", "平均消费", "消费均值", "月均消费", "食堂消费均值", "一卡通消费均值"]),
            "night_net_ratio": _series_from_candidates(frame, ["night_net_ratio", "夜间上网比例", "夜间占比", "night_ratio"]),
            "study_index": _series_from_candidates(frame, ["study_index", "学习指数"]),
            "self_discipline_index": _series_from_candidates(frame, ["self_discipline_index", "自律指数"]),
            "health_index": _series_from_candidates(frame, ["health_index", "健康指数"]),
            "risk_index": _series_from_candidates(frame, ["risk_index", "风险指数"]),
            "development_index": _series_from_candidates(frame, ["development_index", "综合发展指数"]),
            "performance_level": _series_from_candidates(frame, ["performance_level", "表现档次", "综合表现", "performance_label"]),
        }
    )
    ...
    feature_view[numeric_columns] = feature_view[numeric_columns].apply(pd.to_numeric, errors="coerce")
...
def _prepare_model_result(model_path: str | Path | None = None) -> pd.DataFrame:
    ...
    if path is None:
        raise FileNotFoundError("未找到模型结果，请提供 model_result.csv 或 model_outputs/student_labels_and_clusters.csv。")
    ...
    model_view = pd.DataFrame(
        {
            "student_id": frame["student_id"],
            "risk_prob": _series_from_candidates(frame, ["risk_prob", "风险概率", "风险分数", "risk_score", "risk_probability"]),
            "risk_label": _series_from_candidates(frame, ["risk_label", "风险标签", "risk_tag"]),
            "cluster": _series_from_candidates(frame, ["cluster", "聚类类别", "学生画像类别", "cluster_label", "group_cluster"]),
            "performance_level": _series_from_candidates(frame, ["performance_level", "表现档次", "performance_label"]),
        }
    )
...
def _percentile_score(series: pd.Series, *, reverse: bool = False) -> pd.Series:
    numeric = pd.to_numeric(series, errors="coerce")
    ranked = numeric.rank(method="average", pct=True) * 100
    if reverse:
        ranked = 100 - ranked
    return ranked.round(1)


def _display_score_from_series(series: pd.Series, *, prefer_raw_if_score_like: bool = False, reverse: bool = False) -> pd.Series:
    numeric = pd.to_numeric(series, errors="coerce")
    clean = numeric.dropna()
    if clean.empty:
        return pd.Series([np.nan] * len(series), index=series.index, dtype=float)

    if prefer_raw_if_score_like:
        q05 = clean.quantile(0.05)
        q95 = clean.quantile(0.95)
        if q05 >= 0 and q95 <= 100:
            return clean.reindex(series.index).clip(lower=0, upper=100).round(1)

    return _percentile_score(series, reverse=reverse)


def _append_display_scores(result: pd.DataFrame) -> pd.DataFrame:
    result = result.copy()
    result["study_index_display"] = _display_score_from_series(result.get("study_index"), prefer_raw_if_score_like=False)
    result["self_discipline_index_display"] = _display_score_from_series(result.get("self_discipline_index"), prefer_raw_if_score_like=False)
    result["health_index_display"] = _display_score_from_series(result.get("health_index"), prefer_raw_if_score_like=True)
    result["risk_index_display"] = _display_score_from_series(result.get("risk_index"), prefer_raw_if_score_like=False, reverse=True)
    result["development_index_display"] = _display_score_from_series(result.get("development_index"), prefer_raw_if_score_like=True)
    return result
...
def _finalize_master_table(master: pd.DataFrame) -> pd.DataFrame:
    ...
    if result["risk_prob"].isna().all() and "risk_index" in result.columns:
        ...
        result["risk_prob"] = result["risk_prob"].combine_first(_normalize_probability(result["risk_index"]))
    ...
    result["risk_label"] = result["risk_label"].combine_first(inferred)
    ...
    result = _append_display_scores(result)

    for column in MASTER_COLUMNS:
        if column not in result.columns:
            result[column] = np.nan
    return result[MASTER_COLUMNS].sort_values("student_id").reset_index(drop=True)
...
def load_analysis_master(
    ...
    *,
    ...
    if path.exists():
        frame = _clean_columns(_read_csv(path))
        missing_display = any(col not in frame.columns for col in MASTER_COLUMNS)
        if missing_display:
            frame = _finalize_master_table(frame)
        return frame

    if not rebuild_if_missing:
        raise FileNotFoundError(f"未找到分析主表: {path}")
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="构建学生行为分析主表。")
    ...
    parser.add_argument("--output-path", dest="output_path", default=str(DEFAULT_MASTER_PATH), help="analysis_master.csv 输出路径")
    return parser
...
def main() -> None:
    ...
    print(f"分析主表构建完成，共 {len(master)} 条记录。")
